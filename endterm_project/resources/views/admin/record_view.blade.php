        ...
        <div class="col-md-4">
            <div class="card text-dark" style="background-color: #d0e7ff;">
                <div class="card-body">
                    ...
                    <p class="card-text fw-bold">{{ $record->created_at->format('F j, Y g:i A') }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="card text-dark bg-{{ $statusColor }} bg-opacity-25">
                <div class="card-body">
                    ...
                    @if ($record->status == "Failed")
                        {{$record->status}} at {{$record->updated_at->format('F j, Y g:i A')}}
                    @elseif ($record->status == "Completed")
                        {{$record->status}} at {{$record->claimed->format('F j, Y g:i A')}}
                    @else 
                        {{$record->status}} | Last edited: {{$record->updated_at->format('F j, Y g:i A')}}
                    @endif 
                    </p>
                </div>
            </div>
